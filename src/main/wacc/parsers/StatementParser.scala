...
object StatementParser {
    ...
    lazy val _returningIfStmt = If(
      "if" ~> expr,
      "then" ~> _returningBlock,
      "else" ~> _returningBlock <~ "fi"
    )
    lazy val _returningBeginStmt = Begin("begin" ~> _returningBlock <~ "end")
    lazy val _returningStmts: Parsley[Stmt] =
        returnStmt | exitStmt | _returningIfStmt | _returningBeginStmt
    lazy val _returningBlock = Block(
      lift2[List[Stmt], Stmt, List[Stmt]](
        _ :+ _,
        many(atomic(simpleStmt <~ ";")),
        _returningStmts.explain(
    ...
    lazy val pairElem: Parsley[PairElem] = First(
      ("fst" ~> lValue.label("pair"))
