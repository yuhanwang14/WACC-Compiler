...
trait ParserSingletonBridgePos[+R] extends generic.ErrorBridge {
    ...
    infix def from(op: Parsley[?]): Parsley[R] = error(
      pos.map(this.con(_)) <* op
    )
...
trait ParserBridgePos2[-T1, -T2, +R]
    extends ParserSingletonBridgePos[(T1, T2) => R] {
    def apply(t1: T1, t2: T2)(pos: (Int, Int)): R
    def apply(t1: Parsley[T1], t2: => Parsley[T2]): Parsley[R] = error(
      ap2(pos.map(con), t1, t2)
    )

    override final def con(pos: (Int, Int)): (T1, T2) => R =
        this.apply(_, _)(pos)
}

trait ParserBridgePos3[-T1, -T2, -T3, +R]
    extends ParserSingletonBridgePos[(T1, T2, T3) => R] {
    ...
    override final def con(pos: (Int, Int)): (T1, T2, T3) => R =
        this.apply(_, _, _)(pos)
}

trait ParserBridgePos4[-T1, -T2, -T3, -T4, +R]
    extends ParserSingletonBridgePos[(T1, T2, T3, T4) => R] {
    ...
    override final def con(pos: (Int, Int)): (T1, T2, T3, T4) => R =
